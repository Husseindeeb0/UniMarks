...
import { GraduationCap, BookOpen, Award, Loader2, TrendingUp } from "lucide-react";

const StudentMarks = () => {
  const { user } = useAuth();
  const studentId = Number(user?.id);

  const { data: marks, isLoading, error } = useGetStudentMarksQuery(studentId, {
    skip: !studentId,
  });
  ...
  const averageScore =
    validMarks.length > 0
      ? validMarks.reduce((acc, curr) => acc + curr.score, 0) / validMarks.length
      : 0;
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Welcome, <strong className="text-gray-700">{user?.name}</strong>. Here are your official course marks.
              </p>
            </div>
            ...
            <div className="flex space-x-4 sm:space-x-6">
              <div className="bg-blue-50/50 border border-blue-100/50 p-3.5 rounded-2xl flex items-center space-x-3">
                ...
                <div>
                  <p className="text-xs font-semibold text-blue-600/80 uppercase tracking-wider">Courses Graded</p>
                  <p className="text-lg font-bold text-gray-900 leading-tight">{validMarks.length}</p>
                </div>
              </div>
              <div className="bg-indigo-50/50 border border-indigo-100/50 p-3.5 rounded-2xl flex items-center space-x-3">
                ...
                <div>
                  <p className="text-xs font-semibold text-indigo-600/80 uppercase tracking-wider">Average Score</p>
                  <p className="text-lg font-bold text-gray-900 leading-tight">{averageScore.toFixed(1)}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {validMarks.length === 0 ? (
          <div className="bg-white rounded-2xl p-12 text-center border border-gray-100 shadow-sm">
            ...
            <h3 className="text-lg font-bold text-gray-900 mb-1">No marks available yet</h3>
            <p className="text-gray-500 text-sm max-w-sm mx-auto">
              Your teachers haven't uploaded any marks for your courses yet. Check back later!
            </p>
          </div>
        ) : (
                  ...
                  <div className="mt-6 pt-4 border-t border-gray-50 flex items-end justify-between">
                    <div>
                      ...
                      <div className="flex items-baseline space-x-1">
                        <span className="text-3xl font-extrabold text-gray-900">{mark.score}</span>
                        <span className="text-sm font-medium text-gray-500">/ 100</span>
                      </div>
                    </div>
                    
                    {/* Visual indicator for good/bad scores */}
                    <div className={`px-3 py-1 rounded-full text-xs font-bold ${
                      mark.score >= 90 ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' :
                      mark.score >= 70 ? 'bg-blue-100 text-blue-700 border border-blue-200' :
                      mark.score >= 50 ? 'bg-yellow-100 text-yellow-700 border border-yellow-200' :
                      'bg-red-100 text-red-700 border border-red-200'
                    }`}>
                      {mark.score >= 90 ? 'Excellent' :
                       mark.score >= 70 ? 'Good' :
